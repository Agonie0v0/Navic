...
import navic.composeapp.generated.resources.option_status_bar_lyrics_offset
			...
			Column(
				...
				SettingsGroup(title = { Text(stringResource(Res.string.action_lyrics)) }) {
					SettingsNavItem(
						...
						shapes = SegmentedListItemDefaults.segmentedShapes(index = 0, count = 8)
					)
					SettingsSliderItem(
						value = preferenceManager.statusBarLyricsOffsetMs.toFloat(),
						valueRange = -2000f..2000f,
						onValueChange = {
							preferenceManager.statusBarLyricsOffsetMs =
								(it / 100f).roundToInt() * 100
						},
						steps = 39,
						content = { Text(stringResource(Res.string.option_status_bar_lyrics_offset)) },
						trailingContent = {
							val offset = preferenceManager.statusBarLyricsOffsetMs
							Text(if (offset > 0) "+$offset ms" else "$offset ms")
						},
						shapes = SegmentedListItemDefaults.segmentedShapes(index = 1, count = 8)
					)
					SettingsToggleItem(
						...
						shapes = SegmentedListItemDefaults.segmentedShapes(index = 2, count = 8)
					)
					SettingsToggleItem(
						...
						shapes = SegmentedListItemDefaults.segmentedShapes(index = 3, count = 8)
					)
					SettingsToggleItem(
						...
						shapes = SegmentedListItemDefaults.segmentedShapes(index = 4, count = 8)
					)
					SettingsToggleItem(
						...
						shapes = SegmentedListItemDefaults.segmentedShapes(index = 5, count = 8)
					)
					SettingsToggleItem(
						...
						shapes = SegmentedListItemDefaults.segmentedShapes(index = 6, count = 8)
					)
					SettingsToggleItem(
						...
						shapes = SegmentedListItemDefaults.segmentedShapes(index = 7, count = 8)
					)
				}
