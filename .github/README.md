...
GitHub Actions 每 6 小时检查一次上游 `master`。发现新提交后会自动：

1. 将上游版本合并到本仓库。
2. 构建 Android Release APK（不构建 iOS）。
3. 使用本仓库专用证书签名。
4. 发布为 `${上游最新版本号}-statusbar-${本仓库提交短哈希}`，附件名为 `Navic-statusbar.apk`。标签中的上游版本号用于标识最近的上游 Release；安装包还可能包含其后合并的上游 `master` 提交。

手动强制重建会更新同一源码提交对应的安装包。
